...
def conv3x3_bn_relu_deform(in_planes, out_planes, stride=1):
    "3x3 convolution + BN + relu"
    return mySequential(
            torchvision.ops.DeformConv2d(in_planes, out_planes, kernel_size=3,
                      stride=stride, padding=1, bias=False),
            BatchNorm2d(out_planes),
            nn.ReLU(inplace=True),
            )
...
class PPMDeepsup(nn.Module):
    def __init__(self, num_class=150, fc_dim=4096,
        ...
        self.ppm = nn.ModuleList(self.ppm)
        #self.cbr_deepsup = conv3x3_bn_relu(fc_dim // 2, fc_dim // 4, 1)
        self.cbr_deepsup = conv3x3_bn_relu_deform(fc_dim // 2, fc_dim // 4, 1)

        self.conv_last = mySequential(
            torchvision.ops.DeformConv2d(fc_dim+len(pool_scales)*512, 512,
                      kernel_size=3, padding=1, bias=False),
            BatchNorm2d(512),
            nn.ReLU(inplace=True),
            nn.Dropout2d(0.1),
