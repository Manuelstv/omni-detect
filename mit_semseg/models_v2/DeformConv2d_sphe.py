...
def deform_conv2d_pad(
    ...
    if padding_mode != 'zeros':
        ...
        import pdb
        pdb.set_trace()
        _reversed_padding_repeated_twice = torch.nn.modules.utils._reverse_repeat_tuple(padding, 2)
        print(_reversed_padding_repeated_twice)
        padded_input = torch.nn.functional.pad(input, _reversed_padding_repeated_twice, mode=padding_mode)
        padding = (0, 0)
        pad_h, pad_w = _pair(padding)
        print(padding)
        ...
        if isactiv:
            if offset_file == '':
                offset_file = './OFFSETS/offset_'+str(w2)+'_'+str(h2)+'_'+str(self.kernel_size[0])+'_'+str(
                    self.kernel_size[1])+'_'+str(self.stride[0])+'_'+str(self.stride[1])+'_1.pt'
